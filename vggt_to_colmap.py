...
def filter_and_prepare_points(predictions, conf_threshold, mask_sky=False, mask_black_bg=False, 
                             mask_white_bg=False, stride=1, prediction_mode="Depthmap and Camera Branch",output_dir = 'VGGT2COL'):
    """
    ...
    print("Predicted World Points Shape:",pred_world_points.shape)
    ...
    # Step 5: Track which original indices were used
    used_indices = set()
    down_confidences = []
    for voxel_key in voxel_dict:
        indices = voxel_dict[voxel_key]
        voxel_points = filtered_vertices[indices]
        centroid = np.mean(voxel_points, axis=0)
        # Check if this voxel exists in downsampled set
        if np.any(np.all(np.isclose(downsampled_points, centroid, atol=voxel_size), axis=1)):
            used_indices.update(indices)
            # Optionally compute mean/max confidence per voxel
            down_confidences.append(np.mean(filtered_conf[indices]))

    # Step 6: Generate confidence mask
    confidence_mask = np.zeros(len(filtered_vertices), dtype=bool)
    confidence_mask[list(used_indices)] = True

    # Optional: Downsampled confidence values
    down_confidences = np.array(down_confidences)
    print("Trues in original Cong Mask:",sum(conf_mask))
    print("Trues in Filtered Conf Mask:",sum(down_confidences))
    ...
    print("Filtered Vert Shape:",filtered_vertices.shape)
    total_points = 0
...
def main():
    ...
    predictions, image_names = process_images(args.image_dir, model, device,args.fraction)
